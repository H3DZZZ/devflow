...
export async function getRecommendedQuestions({
  ...
  skip,
  query,
}: RecommendationParams): Promise<{ questions: Questions[]; isNext: boolean }> {
  ...
  const interactedQuestions = Question.find({
    _id: { $in: interactedQuestionIds },
  }).select("tags");

  const allTags = (await interactedQuestions).flatMap((q) =>
